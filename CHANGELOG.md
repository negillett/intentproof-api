...
- **CI — ECR releases:** **`docker-ecr-release.yml`** builds **linux/amd64** and pushes to **`intentproof-api`** in ECR on semver tags **`vX.Y.Z`** (OIDC via **`AWS_ECR_PUSH_ROLE_ARN`**); optional **`INTENTPROOF_INFRA_GH_WRITE_TOKEN`** updates **`intentproof-infra`** repository secret **`TF_VAR_IMAGE_TAG`** to the pushed tag after a successful push (optional **`INTENTPROOF_INFRA_REPO`** variable for a non-default infra fork/org).
