...
- **CI — ECR releases:** **`docker-ecr-release.yml`** builds **linux/amd64** and pushes to **`intentproof-api`** in ECR on semver **`vX.Y.Z`** tags (**`AWS_ECR_PUSH_ROLE_ARN`**); optionally syncs **`intentproof-infra`** **`TF_VAR_IMAGE_TAG`** after push via **GitHub App** (**`INTENTPROOF_INFRA_GITHUB_APP_ID`** + **`INTENTPROOF_INFRA_GITHUB_APP_PRIVATE_KEY`**, **`actions/create-github-app-token`**, least-privilege **permission-secrets: write**) or legacy fine-grained **`INTENTPROOF_INFRA_GH_WRITE_TOKEN`**; optional **`INTENTPROOF_INFRA_REPO`** / App owner–repo variables (**`INTENTPROOF_INFRA_GITHUB_APP_*`**).
